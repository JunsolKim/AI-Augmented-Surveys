...
dt <- as.data.table(read_parquet(file.path(IN_DIR, "fig_groupvar_within.parquet")))
sm <- fread(file.path(IN_DIR, "fig_groupvar_within_summary.csv"))

lvls <- c("All", "Age Group", "Gender Group", "Race Group",
...
g <- ggplot(dt, aes(x = pred_std, y = obs_std)) +
  ...
  labs(x = "Predicted within-group SD",
       y = "Observed within-group SD") +
  theme_paper()

out_pdf <- file.path(OUT_DIR, "figure_groupvar_within.pdf")
out_png <- file.path(OUT_DIR, "figure_groupvar_within.png")
