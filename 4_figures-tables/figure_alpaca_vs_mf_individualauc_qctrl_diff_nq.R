...
dt   <- as.data.table(read_parquet(
  file.path(IN_DIR, "alpaca_vs_mf_individualauc_per_respondent.parquet")))
ctrl <- as.data.table(read_parquet(
  file.path(IN_DIR, "_period_controls.parquet")))
...
fit_cell <- function(task_label, model_label) {
  sub <- dt[task == task_label & model == model_label]
  needed <- c("auc","sex","race","degree","region4","income_q5",
              "age_group","year_dec","partyid7","n_q","mean_diff")
  ...
  tidied[, `:=`(task_name = task_label, model_name = model_label)]
  tidied
}

cells <- expand.grid(task = TASK_ORDER, model = MODEL_ORDER, stringsAsFactors = FALSE)
model_out <- rbindlist(Map(fit_cell, cells$task, cells$model), use.names = TRUE)
...
model_out[, model_label := factor(model_name, levels = MODEL_ORDER)]

model_out[, sig := "insig"]
model_out[conf.high < 0 & estimate < 0, sig := "sig"]
model_out[conf.low  > 0 & estimate > 0, sig := "sig"]
model_out[, sig := factor(sig, levels = c("sig", "insig"))]

DODGE <- position_dodge2(width = 0.7, reverse = TRUE)

p <- ggplot(model_out,
            aes(x = estimate * 100, y = term,
                color = model_label, shape = sig)) +
  geom_vline(xintercept = 0, color = "grey") +
  geom_linerange(aes(xmin = 0, xmax = estimate * 100,
                     y = term, group = model_label),
                 linewidth = 0.4, position = DODGE) +
  geom_point(size = 1.2, position = DODGE) +
  ...
  labs(y = NULL, x = "Marginal Effects on individual-level AUC (%)")

out_pdf <- file.path(OUT_DIR, "figure_alpaca_vs_mf_individualauc_qctrl_diff_nq.pdf")
out_png <- file.path(OUT_DIR, "figure_alpaca_vs_mf_individualauc_qctrl_diff_nq.png")
ggsave(out_pdf, p, width = 12, height = 11)
ggsave(out_png, p, width = 12, height = 11, dpi = 300)
cat("Saved:\n  ", out_pdf, "\n  ", out_png, "\n", sep = "")
